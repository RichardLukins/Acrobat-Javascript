...
MergeInterleave = app.trustedFunction(function() {
	try { // start error trapping
		...
		if (this.numPages > 0) { // check there is at least 1 page to work on
			...
			var nRslt = 1;
			var SecondDoc = app.openDoc(filePath);
			var SecondPages = SecondDoc.numPages
			SecondDoc.closeDoc(true);
			if (this.numPages != SecondPages) { // check for matched Page Count
				nRslt = app.alert ("Your Documents have Different Numbers of Pages\n" + "This routine is best suited to same size page counts\n" + "Extra Pages in the Second file will be dropped\n" + "Are you Sure you want to continue?", 3, 1)};
			if (nRslt == 1) {
				var OrigPageCount = this.numPages;
				if (SecondPages < OrigPageCount) {OrigPageCount = SecondPages}
