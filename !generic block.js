...
app.addMenuItem({  cName: "test routine", cParent: "Edit", cExec: "testRoutine();",  cEnable: "event.rc = (event.target != null);", nPos: 0 });
//
// Define the Function
//
testRoutine = app.trustedFunction(function() {
	try { // start error trapping
		...
		if (this.numPages > 0) { // check there is at least 1 page to work on
			var nRslt = 1;
			if (this.numPages != 1) { // check for matched Page Count
