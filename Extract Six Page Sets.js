...
//   ************************************************************************************
//   *****                                                                          *****
//   *****    	EXTRACT ALL PAGES AS Six Pages IN THE DOCUMENT TO PDF FILES         *****
...
app.addMenuItem({  cName: "Extract 6page sets To Own PDF", cParent: "Edit", cExec: "MyExtract6pages();",  cEnable: "event.rc = (event.target != null);", nPos: 0 });
//
// Define the Function
//
MyExtract6pages = app.trustedFunction(function() {
	try { // start error trapping
		...
		if (this.numPages > 0) { // check there is at least 1 page to work on
			var nRslt = 1;
			if (this.numPages%6 == 0) { // check for odd page count
			...
			if (nRslt == 1) {		
				...
				for (var CurPg=0; CurPg<this.numPages; CurPg+=6) { // Pad file name to cope with 19998 pages or 9999 output pages
					...
					tmr.text = 'Extracting pages ' + (CurPg+1) + ' and ' + (CurPg+6) + ' of ' + (this.numPages); // update progress message
					if (CurPg+1 > this.numPages - 5) { // check for last incomplete page range ... if it exists
